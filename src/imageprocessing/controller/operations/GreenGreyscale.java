...
/**
 * Function object that allows the controller to delegate to the model to greyScaleColor green.
 */
public class GreenGreyscale implements ImagesCommand {
  private final String origName;
  private final String copyName;

  /**
   * Constructs the function object to greyScale by the color green, the original name and copy's
   * name are specified.
   *
   ...
   */
  public GreenGreyscale(String orig, String copy) throws NullPointerException {
  ...
  public void run(Map<String, Image> imageStorage) {
    if (imageStorage.get(this.origName).getClass() == ImageInputMock.class) {
      ...
      imageToOperate.greyScaleColorImage("green");
    } else {
